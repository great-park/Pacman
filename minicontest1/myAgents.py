...
class MyAgent(Agent):
    ...
    def getAction(self, state):
        ...
        global start
        if not start:
            food_number = state.getNumFood()
            start = True

        global end
        if len(self.move) != 0:  
            nextmove = self.move.pop(0)
            return nextmove
        else:
            if end == True:
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # True or Flase 일 것.
        (x, y) = state
        # end 설정 -> target_food의 길이 == food_number
        global end
        global food_number
        global target
        # food가 있으면,
        if self.food[x][y] == True and state not in target:
            target.append(state)

            if food_number == len(target):
                # end 조건 만족
                end = True
            return True
        else:
